...
def fine_tune_yolov11n(
    ...
    model = YOLO(str((base_dir / model_path) if not Path(model_path).is_absolute() else model_path))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune YOLOv11n on a dataset")
    ...
    parser.add_argument("--project", dest="project_dir", default=None, help="Project directory for runs")
    parser.add_argument("--name", dest="run_name", default=None, help="Run name")
    args = parser.parse_args()

    weights_path = fine_tune_yolov11n(
