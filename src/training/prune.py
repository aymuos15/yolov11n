...
def prune(args: argparse.Namespace):
    ...
    pruned_model_path = save_dir_before / f"{args.dataset}_batch{args.batch_size}_pruned{int(args.target_prune_rate*100)}_before_finetune.pt"
    torch.save(model.model.state_dict(), pruned_model_path)

    # fine-tuning
    for name, param in model.model.named_parameters():
        param.requires_grad = True
    pruning_cfg['name'] = f"{args.dataset}_batch{args.batch_size}_pruned{int(args.target_prune_rate*100)}"
    pruning_cfg['epochs'] = args.postprune_epochs
    pruning_cfg['batch'] = args.batch_size
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='dataset', type=str, help='Dataset name for output naming')
