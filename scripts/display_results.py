...
def load_metrics_from_runs(runs_dir: Path, dataset: str) -> dict:
    ...
    Returns: {(batch, prune_pct): {precision, recall, mAP50, mAP50-95}}
    """
    ...
    for run_dir in runs_dir.iterdir():
        ...
        # Parse batch and prune_pct from directory name
        if m := pattern_baseline.match(name):
            batch = int(m.group(1))
            prune_pct = 0.0
        elif m := pattern_pruned.match(name):
            batch = int(m.group(1))
            prune_pct = float(m.group(2))
        else:
            continue
...
def display_results(results: list[dict], metrics: dict, dataset: str, console: Console):
    ...
    if results:
        ...
        pruned_maps = [metrics.get((r["batch"], r["prune_pct"]), {}).get("mAP50-95", 0)
    ...
    for batch_size in sorted(batches.keys()):
        ...
        for r in sorted(batch_results, key=lambda x: x["prune_pct"]):
            m = metrics.get((r["batch"], r["prune_pct"]), {})
            map50 = m.get("mAP50", 0)
